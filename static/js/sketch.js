...
function preload() {
	...
	img = loadImage("/static/scaled_image.png"); 
  }
function setup() {
	...
	attempts = 10000;
	message = createInput('Welcome to Visual Media Lab').parent('messageBox');
	text_size = createSlider(1, 150, 150, 1).parent('textSize');
	n = createSlider(1, 1000, 1000, 1).parent('nParticles');
	maxSeekVel = createSlider(1, 10, 5).parent('maxSeekVel');
	maxFleeVel = createSlider(1, 20, 10).parent('maxFleeVel');
	sight = createSlider(1, 60, 50).parent('sight');
